...
namespace fakeit {
    ...
    struct SequenceVerificationEvent : public VerificationEvent {
        ...
        SequenceVerificationEvent(VerificationType aVerificationType, std::vector<Sequence*>& anExpectedPattern,
            ...
            : VerificationEvent(aVerificationType), _expectedPattern(anExpectedPattern),
              _actualSequence(anActualSequence), _expectedCount(anExpectedCount), _actualCount(anActualCount) {
        }
...
namespace fakeit {
    ...
    public:
        ...
        template<typename C> static typename std::enable_if<std::has_virtual_destructor<C>::value, unsigned int>::type
        getDestructorOffset() {
            VirtualOffsetSelector offsetSelctor;
...
namespace fakeit {
    ...
    template<typename R, typename... arglist> struct MethodStubbingProgress {
        ...
        void AlwaysReturn() {
            return AlwaysDo(
                    [](const typename fakeit::test_arg<arglist>::type...) -> R { return DefaultValue<R>::value(); });
        }
...
namespace fakeit {
    ...
    template<typename R, typename... arglist> class MethodMockingContext
        : public Sequence,
          public ActualInvocationsSource,
    ...
    template<typename... arglist> class MockingContext<void, arglist...>
...
namespace fakeit {
    ...
    private:
        ...
        static int findNextMatch(Sequence*& pattern, std::vector<Invocation*>& actualSequence, int startSearchIndex) {
            ...
            for (int i = startSearchIndex; i < (( int ) actualSequence.size() - ( int ) expectedSequence.size() + 1);
                 i++) {
...
namespace fakeit {
    ...
    private:
        ...
        template<typename... list> std::vector<Sequence*>&
        collectSequences(std::vector<Sequence*>& vec, const Sequence& sequence, const list&... tail) {
            vec.push_back(&const_cast<Sequence&>(sequence));
...
namespace fakeit {
    ...
    class UsingProgress {
        ...
        template<typename... list> void collectSequences(std::vector<fakeit::Sequence*>& vec,
...
namespace fakeit {
    ...
    public:
        ...
        template<typename... list> VerifyNoOtherInvocationsVerificationProgress
...
#define ConstOverloadedMethod(mock, method, prototype)                                                                 \
    mock.template stub<__COUNTER__>(CONST_OVERLOADED_METHOD_PTR(mock, method, prototype))                              \
