...
namespace fakeit {
  ...
  template<typename T> class is_ostreamable {
    ...
    template<typename T1> static decltype(operator<<(std::declval<std::ostream&>(), std::declval<const T1>()))
    test(std::ostream& s, const T1& t);
#else
    template<typename T1> static auto test(std::ostream& s, const T1& t) -> decltype(s << t);
#endif
    static no test(...);

  public:
    static const bool value = std::is_arithmetic<T>::value || std::is_pointer<T>::value ||
...
namespace fakeit {
  ...
  template<class C> struct Formatter<C, typename std::enable_if<!is_ostreamable<C>::value>::type> {
  ...
  template<typename T> using TypeFormatter = Formatter<typename fakeit::naked_type<T>::type>;
} // namespace fakeit
...
namespace fakeit {
  ...
  template<class... Args> void print(std::ostream& strm, const std::tuple<Args...>& t) {
    strm << "(";
    TuplePrinter<decltype(t), sizeof...(Args)>::print(strm, t);
    strm << ")";
  }

  template<class... Args> std::ostream& operator<<(std::ostream& strm, const std::tuple<Args...>& t) {
    print(strm, t);
    return strm;
  }
...
namespace fakeit {
  ...
  template<typename... arglist> std::ostream& operator<<(std::ostream& strm, const ActualInvocation<arglist...>& ai) {
    strm << ai.format();
    return strm;
  }
  ...
  private:
    ...
    void filter(std::unordered_set<Invocation*>& source, std::unordered_set<Invocation*>& target) const {
      for (Invocation* i : source) {
        if (shouldInclude(i)) { target.insert(i); }
      }
    }
  };
} // namespace fakeit
...
namespace fakeit {
  ...
  struct NoMoreInvocationsVerificationEvent : public VerificationEvent {
    ...
    NoMoreInvocationsVerificationEvent(std::vector<Invocation*>& allTheIvocations,
                                       std::vector<Invocation*>& anUnverifedIvocations)
        : VerificationEvent(VerificationType::NoMoreInvocations), _allIvocations(allTheIvocations),
  ...
  struct SequenceVerificationEvent : public VerificationEvent {
    ...
    SequenceVerificationEvent(VerificationType aVerificationType, std::vector<Sequence*>& anExpectedPattern,
                              std::vector<Invocation*>& anActualSequence, int anExpectedCount, int anActualCount)
        : VerificationEvent(aVerificationType), _expectedPattern(anExpectedPattern), _actualSequence(anActualSequence),
          _expectedCount(anExpectedCount), _actualCount(anActualCount) {
    }
...
namespace fakeit {
  ...
  private:
    ...
    static void formatExpectedCount(std::ostream& out, fakeit::VerificationType verificationType, int expectedCount) {
      ...
      if (verificationType == fakeit::VerificationType::AtLeast) out << "at least ";
...
namespace fakeit {
  ...
  public:
    DefaultFakeit() : _formatter(), _customFormatter(nullptr), _testingFrameworkAdapter(nullptr) {
    }
...
namespace fakeit {
  ...
  struct NoMoreInvocationsVerificationException : public VerificationException {
    ...
    NoMoreInvocationsVerificationException(std::string format) : _format(format) {
    }
  ...
  struct SequenceVerificationException : public VerificationException {
    SequenceVerificationException(const std::string& format) : _format(format) {
    }
  ...
  public:
    ...
    void fail(std::string vetificationType, Catch::SourceLineInfo sourceLineInfo, std::string failingExpression,
              std::string fomattedMessage,
              Catch::ResultWas::OfType resultWas = Catch::ResultWas::OfType::ExpressionFailed) {
      Catch::AssertionHandler catchAssertionHandler(vetificationType, sourceLineInfo, failingExpression,
    ...
    virtual void handle(const UnexpectedMethodCallEvent& evt) override {
      ...
      fail("UnexpectedMethodCall", ::Catch::SourceLineInfo("Unknown file", 0), "", format,
...
namespace fakeit {
  ...
  template<typename TARGET, typename SOURCE> TARGET union_cast(SOURCE source) {
...
namespace fakeit {
  ...
  class VTUtils {
  public:
    template<typename C, typename R, typename... arglist> static unsigned int getOffset(R (C::*vMethod)(arglist...)) {
    ...
    template<typename C>
    static typename std::enable_if<std::has_virtual_destructor<C>::value, unsigned int>::type getDestructorOffset() {
      VirtualOffsetSelector offsetSelctor;
    ...
    template<typename C>
    static typename std::enable_if<std::has_virtual_destructor<C>::value, bool>::type hasVirtualDestructor() {
      return true;
    }
...
namespace fakeit {
  ...
  template<typename C, typename... baseclasses> struct RTTIClassHierarchyDescriptor {
    ...
    template<typename head, typename B1, typename... tail> void addBaseClass() {
  ...
  template<typename C, typename... baseclasses> struct RTTICompleteObjectLocator {
    ...
    RTTICompleteObjectLocator(const std::type_info& unused)
        : signature(0), offset(0), cdOffset(0), typeDescriptorOffset(0), classDescriptorOffset(0) {
      ( void )unused;
    }
    ...
    RTTICompleteObjectLocator(const std::type_info& info)
        : signature(0), offset(0), cdOffset(0), pTypeDescriptor(&info),
  ...
  template<class C, class... baseclasses> struct VirtualTable : public VirtualTableBase {
...
namespace fakeit {
  template<typename... T1> class has_one_base {};

  template<typename T1, typename T2, typename... types>
  class has_one_base<std::tr2::__reflection_typelist<T1, T2, types...>> : public std::false_type {};

  template<typename T1> class has_one_base<std::tr2::__reflection_typelist<T1>>
      : public has_one_base<typename std::tr2::direct_bases<T1>::type> {};

  template<> class has_one_base<std::tr2::__reflection_typelist<>> : public std::true_type {};

  template<typename T> class is_simple_inheritance_layout
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> struct MethodInvocationHandler : Destructible {
...
#pragma warning(disable : 4200)
  ...
  template<typename C, typename... baseclasses> class FakeObject {
...
namespace fakeit {
  ...
  struct MethodProxy {
    ...
    MethodProxy(unsigned int id, unsigned int offset, void* vMethod) : _id(id), _offset(offset), _vMethod(vMethod) {
    }
...
namespace fakeit {
  ...
  struct InvocationHandlerCollection {
    ...
    static InvocationHandlerCollection* getInvocationHandlerCollection(void* instance) {
      ...
      InvocationHandlerCollection* invocationHandlerCollection =
        ( InvocationHandlerCollection* )vt.getCookie(InvocationHandlerCollection::VT_COOKIE_INDEX);
  ...
  public:
    template<unsigned int id> MethodProxy createMethodProxy(unsigned int offset) {
...
namespace fakeit {
  ...
  template<typename C, typename... baseclasses> struct DynamicProxy {
    ...
    DynamicProxy(C& inst)
        : instance(inst), originalVtHandle(VirtualTable<C, baseclasses...>::getVTable(instance).createHandle()),
          _methodMocks(VTUtils::getVTSize<C>()), _offsets(VTUtils::getVTSize<C>()),
    ...
    template<typename R, typename... arglist> bool isMethodStubbed(R (C::*vMethod)(arglist...)) {
    ...
    template<typename R, typename... arglist> Destructible* getMethodMock(R (C::*vMethod)(arglist...)) {
      auto offset = VTUtils::getOffset(vMethod);
      std::shared_ptr<Destructible> ptr = _methodMocks[offset];
      return ptr.get();
    }
    ...
    void stubDataMember(DATA_TYPE C::*member, const arglist&... initargs) {
      ...
      _members.push_back(std::shared_ptr<DataMemeberWrapper<DATA_TYPE, arglist...>> {
        new DataMemeberWrapper<DATA_TYPE, arglist...>(memberPtr, initargs...)});
    }

    template<typename DATA_TYPE> void getMethodMocks(std::vector<DATA_TYPE>& into) const {
      for (std::shared_ptr<Destructible> ptr : _methodMocks) {
        DATA_TYPE p = dynamic_cast<DATA_TYPE>(ptr.get());
        if (p) { into.push_back(p); }
      }
    }
  ...
  private:
    template<typename DATA_TYPE, typename... arglist> class DataMemeberWrapper : public Destructible {
    private:
      DATA_TYPE* dataMember;

    public:
      DataMemeberWrapper(DATA_TYPE* dataMem, const arglist&... initargs) : dataMember(dataMem) {
    ...
    template<typename DATA_TYPE> DATA_TYPE getMethodMock(unsigned int offset) {
      std::shared_ptr<Destructible> ptr = _methodMocks[offset];
    ...
    template<typename BaseClass> void checkMultipleInheritance() {
      ...
      if (delta > 0) { throw std::invalid_argument(std::string("multiple inheritance is not supported")); }
    }
...
namespace fakeit {
  ...
  struct TupleDispatcher {
    ...
    template<typename TupleType, typename FunctionType> static void
    for_each(TupleType&&, FunctionType&,
    ...
    template<typename TupleType, typename FunctionType> static void for_each(TupleType&& t, FunctionType& f) {
      for_each(std::forward<TupleType>(t), f, std::integral_constant<size_t, 0>());
    }

    template<typename TupleType1, typename TupleType2, typename FunctionType> static void
    for_each(TupleType1&&, TupleType2&&, FunctionType&,
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> struct ActualInvocationHandler : Destructible {
...
namespace fakeit {
  ...
  template<class C> struct is_constructible_type {
    static const bool value = std::is_default_constructible<typename naked_type<C>::type>::value &&
      !std::is_abstract<typename naked_type<C>::type>::value;
  };

  template<class C, class Enable = void> struct DefaultValue;

  template<class C> struct DefaultValue<C, typename std::enable_if<!is_constructible_type<C>::value>::type> {
    static C& value() {
...
namespace fakeit {
  ...
  template<typename T> struct TypedMatcher : IMatcher { virtual bool matches(const T& actual) const = 0; };

  template<typename T> struct TypedMatcherCreator {
  ...
  template<typename T> struct ComparisonMatcherCreator : public TypedMatcherCreator<T> {
  ...
  namespace internal {
    template<typename T> struct TypedAnyMatcher : public TypedMatcherCreator<T> {
    ...
    template<typename T> struct EqMatcherCreator : public ComparisonMatcherCreator<T> {
    ...
    template<typename T> struct GtMatcherCreator : public ComparisonMatcherCreator<T> {
    ...
    template<typename T> struct GeMatcherCreator : public ComparisonMatcherCreator<T> {
    ...
    template<typename T> struct LtMatcherCreator : public ComparisonMatcherCreator<T> {
    ...
    template<typename T> struct LeMatcherCreator : public ComparisonMatcherCreator<T> {
    ...
    template<typename T> struct NeMatcherCreator : public ComparisonMatcherCreator<T> {
  ...
  template<typename T> internal::TypedAnyMatcher<T> Any() {
    internal::TypedAnyMatcher<T> rv;
    return rv;
  }

  template<typename T> internal::EqMatcherCreator<T> Eq(const T& arg) {
    internal::EqMatcherCreator<T> rv(arg);
    return rv;
  }

  template<typename T> internal::GtMatcherCreator<T> Gt(const T& arg) {
    internal::GtMatcherCreator<T> rv(arg);
    return rv;
  }

  template<typename T> internal::GeMatcherCreator<T> Ge(const T& arg) {
    internal::GeMatcherCreator<T> rv(arg);
    return rv;
  }

  template<typename T> internal::LtMatcherCreator<T> Lt(const T& arg) {
    internal::LtMatcherCreator<T> rv(arg);
    return rv;
  }

  template<typename T> internal::LeMatcherCreator<T> Le(const T& arg) {
    internal::LeMatcherCreator<T> rv(arg);
    return rv;
  }

  template<typename T> internal::NeMatcherCreator<T> Ne(const T& arg) {
...
namespace fakeit {
  ...
  template<typename... arglist> struct ArgumentsMatcherInvocationMatcher
    ...
    ArgumentsMatcherInvocationMatcher(const std::vector<Destructible*>& args) : _matchers(args) {
    }
  ...
  private:
    struct MatchingLambda {
      MatchingLambda(const std::vector<Destructible*>& matchers) : _matchers(matchers) {
      }
  ...
  template<typename... arglist> struct UserDefinedInvocationMatcher : ActualInvocation<arglist...>::Matcher {
    virtual ~UserDefinedInvocationMatcher() = default;

    UserDefinedInvocationMatcher(std::function<bool(arglist&...)> match) : matcher {match} {
    }
  ...
  template<typename... arglist> struct DefaultInvocationMatcher : public ActualInvocation<arglist...>::Matcher {
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> class RecordedMethodBody : public MethodInvocationHandler<R, arglist...>,
    ...
    struct MatchedInvocationHandler : ActualInvocationHandler<R, arglist...> {
      ...
      typename ActualInvocation<arglist...>::Matcher& getMatcher() const {
        ...
        typename ActualInvocation<arglist...>::Matcher& matcher =
          dynamic_cast<typename ActualInvocation<arglist...>::Matcher&>(destructable);
        return matcher;
      }
    ...
    buildMatchedInvocationHandler(typename ActualInvocation<arglist...>::Matcher* invocationMatcher,
                                  ActualInvocationHandler<R, arglist...>* invocationHandler) {
    ...
    MatchedInvocationHandler* getInvocationHandlerForActualArgs(ActualInvocation<arglist...>& invocation) {
      for (auto i = _invocationHandlers.rbegin(); i != _invocationHandlers.rend(); ++i) {
        ...
        if (im.getMatcher().matches(invocation)) { return &im; }
      }
      return nullptr;
    }
  ...
  public:
    ...
    R handleMethodInvocation(const typename fakeit::production_arg<arglist>::type... args) override {
      ...
      auto actualInvocation = new ActualInvocation<arglist...>(
        ordinal, method, std::forward<const typename fakeit::production_arg<arglist>::type>(args)...);
...
namespace fakeit {
  ...
  inline QuantifierFunctor operator""
    ...
    if (n != 1) throw std::invalid_argument("Only 1_Time is supported. Use X_Times (with s) if X is bigger than 1");
    return QuantifierFunctor(( int )n);
  }
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> struct RepeatForever : public Action<R, arglist...> {
  ...
  template<typename R, typename... arglist> struct ReturnDefaultValue : public Action<R, arglist...> {
  ...
  template<typename R, typename... arglist> struct ReturnDelegateValue : public Action<R, arglist...> {

    ReturnDelegateValue(std::function<R(const typename fakeit::test_arg<arglist>::type...)> delegate)
        : _delegate(delegate) {
    }
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> struct MethodStubbingProgress {
    ...
    MethodStubbingProgress<R, arglist...>& Return(const first& f, const second& s, const tail&... t) {
      Return(f);
      return Return(s, t...);
    }

    template<typename U = R>
    typename std::enable_if<!std::is_reference<U>::value, void>::type AlwaysReturn(const R& r) {
      return AlwaysDo([r](const typename fakeit::test_arg<arglist>::type...) -> R { return r; });
    }

    template<typename U = R> typename std::enable_if<std::is_reference<U>::value, void>::type AlwaysReturn(const R& r) {
      return AlwaysDo([&r](const typename fakeit::test_arg<arglist>::type...) -> R { return r; });
    }
    ...
    template<typename E> MethodStubbingProgress<R, arglist...>& Throw(const E& e) {
      return Do([e](const typename fakeit::test_arg<arglist>::type...) -> R { throw e; });
    }

    template<typename E> MethodStubbingProgress<R, arglist...>& Throw(const Quantifier<E>& q) {
    ...
    template<typename first, typename second, typename... tail>
    MethodStubbingProgress<R, arglist...>& Throw(const first& f, const second& s, const tail&... t) {
      Throw(f);
      return Throw(s, t...);
    }
    ...
    template<typename F> MethodStubbingProgress<R, arglist...>& Do(const Quantifier<F>& q) {
      return DoImpl(new Repeat<R, arglist...>(q.value, q.quantity));
    }

    template<typename first, typename second, typename... tail>
    MethodStubbingProgress<R, arglist...>& Do(const first& f, const second& s, const tail&... t) {
      Do(f);
      return Do(s, t...);
    }
  ...
  template<typename... arglist> struct MethodStubbingProgress<void, arglist...> {
    ...
    Do(std::function<void(const typename fakeit::test_arg<arglist>::type...)> method) {
      return DoImpl(new Repeat<void, arglist...>(method));
    }

    void AlwaysReturn() {
      return AlwaysDo(
        [](const typename fakeit::test_arg<arglist>::type...) -> void { return DefaultValue<void>::value(); });
    }
    ...
    template<typename E> MethodStubbingProgress<void, arglist...>& Throw(const E& e) {
      return Do([e](const typename fakeit::test_arg<arglist>::type...) -> void { throw e; });
    }

    template<typename E> MethodStubbingProgress<void, arglist...>& Throw(const Quantifier<E>& q) {
      const E& value = q.value;
    ...
    template<typename first, typename second, typename... tail>
    MethodStubbingProgress<void, arglist...>& Throw(const first& f, const second& s, const tail&... t) {
      Throw(f);
      return Throw(s, t...);
    }
    ...
    template<typename F> MethodStubbingProgress<void, arglist...>& Do(const Quantifier<F>& q) {
      return DoImpl(new Repeat<void, arglist...>(q.value, q.quantity));
    }

    template<typename first, typename second, typename... tail>
    MethodStubbingProgress<void, arglist...>& Do(const first& f, const second& s, const tail&... t) {
      Do(f);
      return Do(s, t...);
    }
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> struct ActionSequence : ActualInvocationHandler<R, arglist...> {
...
namespace fakeit {
  ...
  template<typename C, typename DATA_TYPE> class DataMemberStubbingRoot {
  private:
  public:
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> struct StubbingContext : public Xaction {
    virtual void appendAction(Action<R, arglist...>* action) = 0;
  };
} // namespace fakeit
#include <functional>
...
namespace fakeit {
  ...
  public:
    template<std::size_t N> using ArgType = typename std::tuple_element<N, std::tuple<arglist...>>::type;

    template<std::size_t N> using NakedArgType = typename naked_type<ArgType<index>>::type;

    template<std::size_t N> using ArgMatcherCreatorType =
    ...
    template<typename Head> typename std::enable_if<std::is_constructible<NakedArgType<index>, Head>::value, void>::type
    ...
    typename std::enable_if<std::is_constructible<NakedArgType<index>, Head>::value, void>::type
    ...
    typename std::enable_if<std::is_base_of<TypedMatcherCreator<NakedArgType<index>>, Head>::value, void>::type
    ...
    template<typename Head>
    typename std::enable_if<std::is_same<AnyMatcher, Head>::value, void>::type CollectMatchers(const Head&) {
      TypedMatcher<NakedArgType<index>>* d = Any<NakedArgType<index>>().createMatcher();
      _matchers.push_back(d);
    }

    template<typename Head, typename... Tail> typename std::enable_if<std::is_same<AnyMatcher, Head>::value, void>::type
    CollectMatchers(const Head& head, const Tail&... tail) {
      CollectMatchers(head);
      MatchersCollector<index + 1, arglist...> c(_matchers);
      c.CollectMatchers(tail...);
...
namespace fakeit {
  ...
  template<typename R, typename... arglist> class MethodMockingContext : public Sequence,
                                                                         public ActualInvocationsSource,
  ...
  private:
    ...
    public:
      Implementation(Context* stubbingContext)
          : _stubbingContext(stubbingContext), _recordedActionSequence(new ActionSequence<R, arglist...>()),
            _invocationMatcher {new DefaultInvocationMatcher<arglist...>()}, _commited(false) {
      }
      ...
      void getActualInvocations(std::unordered_set<Invocation*>& into) const {
        auto scanner = [&](ActualInvocation<arglist...>& a) {
          if (_invocationMatcher->matches(a)) { into.insert(&a); }
        };
      ...
      bool matches(Invocation& invocation) {
        ...
        if (!getStubbingContext().isOfMethod(actualMethod)) { return false; }
  ...
  protected:
    MethodMockingContext(Context* stubbingContext) : _impl {new Implementation(stubbingContext)} {
    }

    MethodMockingContext(MethodMockingContext&) = default;

    MethodMockingContext(MethodMockingContext&& other) : _impl(std::move(other._impl)) {
    }
    ...
    template<class... matcherCreators,
             class = typename std::enable_if<sizeof...(matcherCreators) == sizeof...(arglist)>::type>
  ...
  template<typename R, typename... arglist> class MockingContext : public MethodMockingContext<R, arglist...> {
  ...
  public:
    ...
    MockingContext(MockingContext&& other) : MethodMockingContext<R, arglist...>(std::move(other)) {
    }
    ...
    template<class... arg_matcher> MockingContext<R, arglist...>& Using(const arg_matcher&... arg_matchers) {
    ...
    template<typename U = R> typename std::enable_if<!std::is_reference<U>::value, void>::type operator=(const R& r) {
  ...
  template<typename... arglist> class MockingContext<void, arglist...> : public MethodMockingContext<void, arglist...> {
    MockingContext& operator=(const MockingContext&) = delete;
  ...
  public:
    ...
    MockingContext(MockingContext&& other) : MethodMockingContext<void, arglist...>(std::move(other)) {
    }
    ...
    template<class... arg_matcher> MockingContext<void, arglist...>& Using(const arg_matcher&... arg_matchers) {
...
namespace fakeit {
  ...
  template<typename C, typename... baseclasses> class MockImpl : private MockObject<C>,
                                                                 public virtual ActualInvocationsSource {
  public:
    MockImpl(FakeitContext& fakeit, C& obj) : MockImpl<C, baseclasses...>(fakeit, obj, true) {
    }

    MockImpl(FakeitContext& fakeit) : MockImpl<C, baseclasses...>(fakeit, *(createFakeInstance()), false) {
  ...
  private:
    ...
    template<typename R, typename... arglist> class MethodMockingContextBase
    ...
    template<typename R, typename... arglist> class MethodMockingContextImpl
    ...
    template<int id, typename R, typename... arglist> class UniqueMethodMockingContextImpl
    ...
    protected:
      virtual RecordedMethodBody<R, arglist...>& getRecordedMethodBody() override {
        return MethodMockingContextBase<R, arglist...>::_mock.template stubMethodIfNotStubbed<id>(
          MethodMockingContextBase<R, arglist...>::_mock._proxy, MethodMockingContextImpl<R, arglist...>::_vMethod);
      }
    ...
    public:
      ...
      DtorMockingContextImpl(MockImpl<C, baseclasses...>& mock) : MethodMockingContextBase<void>(mock) {
      }
    ...
    static MockImpl<C, baseclasses...>* getMockImpl(void* instance) {
      ...
      MockImpl<C, baseclasses...>* mock =
        reinterpret_cast<MockImpl<C, baseclasses...>*>(fake->getVirtualTable().getCookie(1));
      return mock;
    }

    bool isOwner() {
      return _instanceOwner != nullptr;
    }
    ...
    static C* createFakeInstance() {
      ...
      if (VTUtils::hasVirtualDestructor<C>()) fake->setDtor(unmockedDtorStubPtr);
      return reinterpret_cast<C*>(fake);
    }

    template<typename R, typename... arglist> void* getOriginalMethod(R (C::*vMethod)(arglist...)) {
    ...
    template<unsigned int id, typename R, typename... arglist> RecordedMethodBody<R, arglist...>&
    stubMethodIfNotStubbed(DynamicProxy<C, baseclasses...>& proxy, R (C::*vMethod)(arglist...)) {
    ...
    RecordedMethodBody<void>& stubDtorIfNotStubbed(DynamicProxy<C, baseclasses...>& proxy) {
      if (!proxy.isDtorStubbed()) { proxy.stubDtor(createRecordedDtorBody(*this)); }
      Destructible* d = proxy.getDtorMock();
      RecordedMethodBody<void>* dtorMock = dynamic_cast<RecordedMethodBody<void>*>(d);
      return *dtorMock;
    }

    template<typename R, typename... arglist> static RecordedMethodBody<R, arglist...>*
...
namespace fakeit {
  ...
  template<int X, typename R, typename C, typename... arglist> struct UniqueMethod {
    R (C::*method)(arglist...);
...
namespace fakeit {
  ...
  template<typename C, typename... baseclasses> class Mock : public ActualInvocationsSource {
  ...
  public:
    ...
    template<int id, typename R, typename T, typename... arglist,
             class = typename std::enable_if<!std::is_void<R>::value && std::is_base_of<T, C>::value>::type>
...
namespace fakeit {
  ...
  public:
    template<typename R, typename... arglist> struct MethodProgress : MethodStubbingProgress<R, arglist...> {

      friend class WhenFunctor;

      virtual ~MethodProgress() override = default;

      MethodProgress(MethodProgress& other) : _progress(other._progress), _context(other._context) {
      }
...
namespace fakeit {
  ...
  class FakeFunctor {
  private:
    template<typename R, typename... arglist> void fake(const StubbingContext<R, arglist...>& root) {
  ...
  public:
    template<typename H, typename... M> void operator()(const H& head, const M&... tail) {
      fake(head);
      this->operator()(tail...);
    }
  };
  ...
  struct InvocationUtils {
    ...
    static void selectNonVerifiedInvocations(std::unordered_set<Invocation*>& actualInvocations,
      ...
      for (auto invocation : actualInvocations) {
        if (!invocation->isVerified()) { into.insert(invocation); }
      }
    }
    ...
    template<typename... list> static void collectInvocationSources(std::vector<ActualInvocationsSource*>& into,
...
namespace fakeit {
  ...
  private:
    ...
    static bool findNextMatch(std::vector<Sequence*>& pattern, std::vector<Invocation*>& actualSequence,
                              int startSearchIndex, int& end, std::vector<Invocation*>& matchedInvocations) {
    ...
    static bool isMatch(std::vector<Invocation*>& actualSequence, std::vector<Invocation::Matcher*>& expectedSequence,
                        int start) {
      bool found = true;
    ...
    static int findNextMatch(Sequence*& pattern, std::vector<Invocation*>& actualSequence, int startSearchIndex) {
      ...
      for (int i = startSearchIndex; i < (( int )actualSequence.size() - ( int )expectedSequence.size() + 1); i++) {
        if (isMatch(actualSequence, expectedSequence, i)) { return i; }
      }
      return -1;
    }
  };
...
namespace fakeit {
  ...
  private:
    ...
    SequenceVerificationExpectation(VerificationEventHandler& fakeit, InvocationsSourceProxy mocks,
                                    std::vector<Sequence*>& expectedPattern)
        : _fakeit(fakeit), _involvedInvocationSources(mocks), _expectedPattern(expectedPattern), _expectedCount(-1),
          _line(0), _isVerified(false) {
    }
    ...
    template<typename... list> std::vector<Sequence*>& collectSequences(std::vector<Sequence*>& vec,
...
namespace fakeit {
  ...
  class SequenceVerificationProgress {
    ...
    SequenceVerificationProgress(FakeitContext& fakeit, InvocationsSourceProxy sources,
...
namespace fakeit {
  ...
  public:
    UsingProgress(fakeit::FakeitContext& fakeit, InvocationsSourceProxy source) : _fakeit(fakeit), _sources(source) {
    }
...
namespace fakeit {
  ...
  public:
    ...
    template<typename... list> UsingProgress operator()(const ActualInvocationsSource& head, const list&... tail) {
...
namespace fakeit {
  ...
  public:
    ...
    template<typename... list> SequenceVerificationProgress operator()(const Sequence& sequence, const list&... tail) {
...
namespace fakeit {
  ...
  class SpyFunctor {
  private:
    template<typename R, typename... arglist> void spy(const SpyingContext<R, arglist...>& root) {
...
#define Method(mock, method) mock.template stub<__COUNTER__>(&MOCK_TYPE(mock)::method).setMethodDetails(#mock, #method)
...
#define Verify(...) Verify(__VA_ARGS__).setFileInfo(__FILE__, __LINE__, __func__)

#define Using(...) Using(__VA_ARGS__)

#define VerifyNoOtherInvocations(...) VerifyNoOtherInvocations(__VA_ARGS__).setFileInfo(__FILE__, __LINE__, __func__)
